...
3. Pick the runners. Use `arena runners` from the current harness's pstack model sheet when present. Otherwise default to one each on the entries of the `arena runners` row in the role table of `provider-dispatch.md`. Spawn more when the arena covers multiple design directions. Same descriptor N times when the work is generation-bound rather than judgment-sensitive.

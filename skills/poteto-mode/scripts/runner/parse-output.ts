...
export function parseProviderOutput(
  ...
  switch (cliFor(provider)) {
    ...
    default:
      throw new Error(`no output parser for provider ${provider}`);
  }
}

/**
 * Does the provider-reported model prove that the requested family was served?
 * The family's `reportedModel` pattern in model-matrix.json decides. A family
 * with a null pattern (Codex) never verifies by report; a pair outside the
 * matrix never verifies at all.
 */
export function reportedModelMatches(
  ...
  return familyReportMatches(family, reported);
}

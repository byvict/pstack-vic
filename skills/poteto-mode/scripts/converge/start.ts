...
interface LaunchReceipt {
  ...
  model: 'grok-4.7'; effort: Effort; modelSelection: string;
...
async function recover(directory: string, key: string, expected: { repo: string; pr: number; toolingRef: string; effort: Effort }): Promise<LaunchReceipt> {
  const intent = object(JSON.parse(readFileSync(resolve(directory, 'intent.json'), 'utf8')));
  const head = sha(intent.head);
  if (intent.repo !== expected.repo || intent.pr !== expected.pr || intent.toolingRef !== expected.toolingRef || intent.effort !== expected.effort) throw new Error('Existing launch intent belongs to different inputs');
  ...
  const receipt = parseReceipt({ schemaVersion: 1, repo: expected.repo, pr: expected.pr, head, toolingRef: expected.toolingRef, model: 'grok-4.7', effort: expected.effort,
...
function ownerPrompt(options: { repo: string; pr: number; head: string; toolingRef: string; effort: Effort }): string {
  const { repo, pr, head, toolingRef, effort } = options;
...
For each current head, run converge-reconcile from the pinned tooling. Prepare its one 'pr verifier' lane with descriptor cursor:grok-4.7@high for a simple change or cursor:grok-4.7@xhigh for a complex change. Launch that lane with pstack-runner --parent codex --provider cursor --model grok-4.7 --effort high or xhigh --mode read-only --repo ${repo} --pr ${pr}. Use the prompt, output and receipt paths from the manifest. Run publish.ts with the report, manifest and evidence directory. Read its machine verdict and exact-head check run from the Cursor App. Do not end your run after publication.

If NOT VERIFIED, inspect the findings and fix the branch yourself. Confirm the live head before push. Record the attempt, push, wait for relevant CI, then run a fresh reconciliation and a fresh independent verifier. If INCONCLUSIVE, recover the named missing condition and repeat verification without counting a code repair. Do not reuse a verdict for a changed patch. For a complex repair discovered while this owner is high, launch a replacement Grok 4.7 xhigh owner with a durable handoff and end only after its launch is confirmed; never perform complex work at high.
...
export async function start(options: { repo: string; pr: number; toolingRef: string; stateDirectory?: string; effort: Effort; configPath?: string }): Promise<LaunchReceipt> {
  ...
  try {
    ...
    if (existing.repo !== repo || existing.pr !== options.pr || existing.toolingRef !== toolingRef || existing.effort !== options.effort) throw new Error('Existing launch belongs to different inputs');
    return existing;
  ...
  const selected = selectCursorModel(await cursor('/v1/models', key), 'grok-4.7', options.effort);
  mkdirSync(directory, { recursive: true, mode: 0o700 });
  const prompt = ownerPrompt({ repo, pr: options.pr, head: initial.head, toolingRef, effort: options.effort });
  const intent = { schemaVersion: 1, repo, pr: options.pr, head: initial.head, toolingRef, model: 'grok-4.7', effort: options.effort, modelSelection: selected.evidence, prompt, createdAt: new Date().toISOString() };
  ...
  const receipt = parseReceipt({ schemaVersion: 1, repo, pr: options.pr, head: initial.head, toolingRef, model: 'grok-4.7', effort: options.effort, modelSelection: selected.evidence,
...
function parseReceipt(value: unknown): LaunchReceipt {
  ...
  return { schemaVersion: 1, repo: repoName(v.repo), pr, head: sha(v.head), toolingRef: sha(v.toolingRef), model: 'grok-4.7', effort: v.effort, modelSelection: string(v.modelSelection), agentId, runId, agentUrl };
}

if (import.meta.main) {
  try {
    const { values } = parseArgs({ options: { repo: { type: 'string' }, pr: { type: 'string' }, 'tooling-ref': { type: 'string' }, state: { type: 'string' }, effort: { type: 'string' }, config: { type: 'string' } } });
    if (!values.repo || !values.pr || !/^\d+$/.test(values.pr) || !values['tooling-ref'] || !['high', 'xhigh'].includes(values.effort ?? '')) throw new Error('Usage: start.ts --repo owner/repo --pr N --tooling-ref SHA --effort high|xhigh [--state directory]');
    process.stdout.write(JSON.stringify(await start({ repo: values.repo, pr: Number(values.pr), toolingRef: values['tooling-ref'], stateDirectory: values.state, effort: values.effort === 'xhigh' ? 'xhigh' : 'high', configPath: values.config }), null, 2) + '\n');

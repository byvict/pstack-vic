...
function certifiedPr(f: ReturnType<typeof fixture>, body = '## Verification\ncertificate: pre-pr\n') {
  const config = JSON.parse(f.state.blobs['.cursor/converge.json']);
  config.prePr = { runs: [{ name: 'suite', command: 'true' }], certifier: false };
  f.state.blobs['.cursor/converge.json'] = JSON.stringify(config); f.state.body = body; f.save();
  const checkout = f.checkout();
  const run = join(f.directory, 'run');
  assert.equal(f.run('converge-certify', ['run', '--directory', run, '--name', 'suite', '--cwd', checkout, '--', 'true']).status, 0);
...
for (const fault of ['lane-without-certificate', 'retain-without-certificate', 'lane-with-certificate', 'converge-report', 'ci-claim', 'edited-log'] as const) {
  test(`certificate publication refuses ${fault} before any write`, t => {
    const f = fixture(); t.after(f.cleanup);
    const run = fault === 'ci-claim' ? certifiedPr(f, f.state.body) : certifiedPr(f);

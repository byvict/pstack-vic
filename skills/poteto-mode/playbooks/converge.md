...
The owner uses Node 24, `gh`, the pinned pstack commit, and its run-scoped Cursor key. It writes state before dispatch or branch mutation. Stop after two code repair attempts or six hours. CI waits and environment recovery do not spend a repair attempt. On an unknown writer outcome, recover the recorded agent/run before another writer. If the cycle cannot advance, post the cause, attempts and evidence, then apply `needs-victor`.

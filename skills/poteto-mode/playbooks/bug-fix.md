...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured `bug-fix` role (default: its row in the role table of [`../references/provider-dispatch.md`](../references/provider-dispatch.md)) with `isolated-write`, a dedicated worktree, and a specific scope. Review the diff.

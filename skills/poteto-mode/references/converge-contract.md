...
`converge-arm` checks the current trunk Tests run, effective branch protection and branch rules, every required check, the authenticated exact-head verdict and human hold labels. `verdict` must be required in protection and bound to the Cursor App before activation. It uses squash auto-merge with `--match-head-commit`. A changed head, red check, missing protection, `needs-victor`, or unreadable state refuses merge. The owner monitors the pending request and disarms on a new hold, invalid verdict, lost ownership or timeout. After merge it watches the resulting main Tests run. A held `verdict-only` proof posts an `action_required` check and never arms.

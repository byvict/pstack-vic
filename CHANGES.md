...
- **Worktree e `checkout`.** `--cwd` precisa estar dentro de um worktree git (`git rev-parse --show-toplevel`); senão o runner recusa antes do preflight. O recibo ganha `checkout: { headBefore, headAfter, statusAfter } | null`: o HEAD antes de o filho do modelo começar, o HEAD depois e as linhas de `git status --porcelain --untracked-files=all` depois. A leitura de depois fica num `finally`, então vale também para prazo, cancelamento e exceção. `checkout` é `null` nos outros modos, numa lane HTTP e quando a lane não chega ao filho do modelo. O git roda com `-C <cwd>`, `--no-optional-locks` e sem as variáveis `GIT_*` herdadas, para que um `GIT_DIR` do pai não aponte o registro para outro repositório. Se o git falha depois do filho, o recibo sai `child-failed` com `checkout: null`. O runner só registra: uma lane que mexeu no worktree ainda sai `complete`. `schemaVersion` continua 1.
...
- `npm test`: TESTS_TOTAL testes, 0 falhas. São 13 testes novos: 3 em `commands.test.ts`, 1 em `cli.test.ts`, 7 em `run.test.ts` (Grok falso num repositório git de fixture) e 2 em `evidence.test.ts`. Antes do código, `commands.test.ts` e `cli.test.ts` falhavam no import de `configOverlay` e no parse do modo. Em `run.test.ts` falhavam 10 testes: 6 novos e os 4 que passaram a exigir `checkout: null`. Em `evidence.test.ts` falhavam 3 e em `certify.test.ts` 5, porque os fixtures agora dão ao Certificador um recibo `unsandboxed`. O teste do revisor já passava, e guarda o que não muda. O sétimo teste de `run.test.ts`, uma lane que apaga o `.git` do worktree, veio depois do código. Ele falhou contra a primeira versão, que perdia o `exitCode` do filho quando o git falhava depois dele, e passa desde que o exit entra na evidência antes da leitura do checkout.
- TESTBUN_LINE `npm run matrix:check`, `npm run agents:check`, `npm run collision:check`, `claude plugin validate .` e `git diff --check` limpos.

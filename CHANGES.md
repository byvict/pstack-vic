...
Parte 2 do plano [`docs/superpowers/plans/2026-09-24-pre-pr.md`](docs/superpowers/plans/2026-09-24-pre-pr.md), sobre a spec [`docs/pre-pr.md`](docs/pre-pr.md). A Raiz arma o auto-merge logo depois de publicar o Certificado, sem esperar o CI do PR. O Varredor arma, depois de cada `Tests` na `main`, todo PR certificado que ainda não está armado. O lançador do owner de "PR opened" encerra quando o head já tem `verdict` confiável. A referência está em `skills/poteto-mode/references/converge-contract.md`.

## Desenho

- **`converge-arm --pending` arma com o CI do PR ainda rodando.** Não exige o run do Tests do head exato concluído nem cada check obrigatório verde. Um check obrigatório na fila, em andamento, esperando ou ainda ausente passa, e o GitHub segura o merge até ele passar. Um check obrigatório concluído com qualquer conclusão diferente de `success` recusa (`Required protected check failed: NAME`), inclusive `neutral` e `skipped`, que o modo estrito também recusa. O resto é igual ao modo estrito: Tests e job de teste na ponta da `main`, todo contexto do contrato na proteção efetiva, `verdict` confiável no head exato, PR aberto e pronto, com base `main`, no head dado e sem hold. Sem `--pending`, o arm não muda.
- **Um Certificado montado numa ponta anterior da `main` é derivado de novo na ponta atual.** O commit de contrato do Certificado é a ponta da `main` no `assemble`, e o Varredor roda depois de um push na `main`, então a ponta quase sempre mudou. Antes, o arm recusava todo `verdict` com commit de contrato diferente da ponta, e o Varredor recusaria todo Certificado. Agora, para um dossiê `pre-pr`, `certifiedAtTip` em `arm.ts` monta o snapshot do PR sob `pre-pr` na ponta atual, roda a mesma análise com o id do round e exige o mesmo head, o mesmo patch id estável e o mesmo digest de verificação (`Certificate patch or policy differs at trunk tip SHA`). Depois decide de novo com uma lane retida por papel exigido, com a cobertura e a adjudicação de risco do dossiê, como o `publish.ts --retain` faz, e recusa o que não sair VERIFIED (`Certificate is no longer VERIFIED at trunk tip SHA: VERDICT`). Uma ponta que muda durante a leitura recusa (`Trunk moved before arm`). Nada é publicado. Vale no modo estrito e no `--pending`, porque depende da execução, não do modo. Um dossiê `converge` com commit de contrato antigo continua recusado, e o owner da nuvem republica por `--retain`. A forma da lane retida saiu para `retainedLanes` em `publish.ts`, que o `--retain` e o arm usam.
- **`converge-sweep --repo OWNER/REPO [--config PATH] [--dry-run]`.** Lista os PRs abertos com base na `main`, em ordem de número. Pula base diferente da `main`, rascunho, hold, auto-merge já pendente e head sem `verdict` confiável VERIFIED. Roda `converge-arm --pending` em cada um dos outros. Imprime `{ swept: [{ pr, head, outcome, reason }] }`, com `outcome` `armed`, `dry-run`, `skipped` ou `refused`. Uma recusa, ou uma falha ao ler o status de um PR, entra como `refused` e não para a varredura; a saída é 1 quando algum PR foi recusado. O Varredor não julga código e não comenta no PR. A Automation da parte 4 decide o que fazer com uma recusa. Um filho de stack não tem base `main`, então espera o pai mergear e o GitHub retargetar.
- **`start.ts` sai sem lançar owner quando o head está certificado.** Depois de admitir o PR, se o `verdict` mais novo do head é `success`, `VERIFIED by converge` e da conta autenticada, devolve `{ schemaVersion: 1, kind: 'certified', repo, pr, head, verdictUrl }` sem chamar o Cursor e sem gravar intenção de lançamento. Um `verdict` que não é VERIFIED, ou de outra conta, ainda lança o owner. Recibo e intenção de lançamento já gravados continuam valendo antes dessa leitura.
...
- `npm test`: 497 testes, 0 falhas. A suíte do Converge tem 219, e 22 são desta parte: 15 em `arm.test.ts`, 4 em `sweep.test.ts` e 3 em `start.test.ts`.
...
- Os testes rodam os CLIs reais contra o `gh` falso. O `--pending` arma com checks na fila e o Tests do head ainda rodando, com a mutação exata `gh pr merge 1 --repo Example/app --squash --auto --match-head-commit HEAD`, onde o modo estrito recusa. Recusa `failure` e `neutral`. Sem checks ainda, recusa sem mutação hold, `main` vermelha, `verdict` fora da proteção, `verdict` não confiável, corpo alterado e base diferente da `main` (foco de revisão 4). Com `tests.job: 'test'`, o arm aceita um job `test` na `main`, que o contrato padrão recusa.
- Um Certificado `ci-only` e um `full` publicados antes de a `main` andar passam depois no arm estrito em dry-run e armam no `--pending`. Com a `main` andando, recusam sem mutação: um arquivo de política trocado, um arquivo sem patch legível (decisão não VERIFIED) e um dossiê `converge`. O Varredor arma o Certificado publicado antes de a `main` andar, e o `--dry-run` dele não faz mutação. Arma o PR certificado e pula o filho de stack, o com hold, o já armado e o rascunho; pula o PR sem `verdict` com saída 0; e com a `main` vermelha devolve `refused` e saída 1.

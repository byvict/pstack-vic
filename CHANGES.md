...
- **O Certificado fica na máquina local**, na pasta da execução, com as Corridas, os recibos e os artefatos. O dossiê publicado carrega o round do PR e o Certificado, na chave nova `certificate`. Um dossiê `pre-pr` precisa dele, preso ao mesmo head e ao mesmo `evidenceDigest`; `converge` e `verdict-only` publicam `certificate: null`, com os mesmos bytes de antes mais essa chave. Um dossiê publicado antes, sem a chave, é lido como `null`, então o `converge-arm` continua lendo os comentários antigos.

...
- **Publicação.** Um relatório `pre-pr` só publica por `publish.ts --certificate`, sem `--lane` e sem `--retain`. Com `--certificate`, um corpo de PR com claims `check:`, `test:` ou `artifact:` é recusado antes de qualquer escrita, porque a publicação roda com o CI ainda pendente. Claims `feature:` são aceitas e só passam quando o Certificado cobre a funcionalidade. O `publish.ts` readmite cada lane e cada Corrida a partir dos bytes, recusa head, patch, contrato ou política diferentes dos do Certificado e posta o mesmo comentário versionado e o mesmo status `verdict` (`VERIFIED by converge`). Sob `pre-pr`, as lacunas de CI são ignoradas, e o snapshot do PR deixa as fontes do runner de testes do CI fora do digest de política.
- **O Certificado fica na máquina local**, na pasta da execução. O dossiê publicado aponta para ele pelo `evidenceDigest` e pelo round do PR.
...
- `npm test`: 453 testes, 0 falhas. A suíte do Converge tem 175, e 42 são desta parte: 17 em `certify.test.ts`, 11 em `publish.test.ts`, 5 em `contract.test.ts`, 5 em `evidence.test.ts` e 4 em `reconcile.test.ts`.

...
- **Pontos de "A medir" da spec:**
  ...
  2. `claude -p` aninhado: inconclusivo. Neste Mac o `claude` do npm está com a sessão OAuth vencida (`auth status` dá `loggedIn: false` até com o ambiente zerado, e `claude -p` responde "OAuth session expired and could not be refreshed"). A limpeza de `CLAUDECODE`/`CLAUDE_CODE_*` não é a causa. Medir de novo depois do login.
  3. A lane `seatbelt` do claude parou no preflight pelo mesmo motivo, então ainda não se sabe se ela escreve fora das raízes. Na lane `seatbelt` do grok, as únicas negações foram de `/dev`.
...
- Sonda real nas versões instaladas: codex 0.155.1 3/3 (30 s), grok 1.0.5 3/3 (31 s). No claude 2.1.281, `manifest` passou e as três lanes com modelo caíram em `unauthenticated` (o login vencido acima).

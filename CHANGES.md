...
- **Prova pelo runner.** Worktree descartável `~/Dev/clinext-4a-proof` do Clinext em `11b09bbae`, com as dependências clonadas por `cp -Rc` (os lockfiles eram iguais). O prompt é o do fluxo medido na CLI-197, com caminhos e portas novos (3475 e 5475) e um comando 8 que lista só os nomes das variáveis do shell. Com `pstack-runner --parent claude --provider grok --model grok-4.7 --effort high --mode unsandboxed --timeout 900`, a lane terminou em 80 s com `LAUNCH=0 DOCTOR=0 DRIVE=0 CLEANUP=0` e PNG e JSON em `<evidência>/login`. O recibo saiu `complete`, com `reportedModel: grok-4.7-build`, `checkout.headBefore` e `checkout.headAfter` em `11b09bbae` e `statusAfter: []`. Sessão Grok `01a0d925-f116-7f63-8b1b-522736525367`. Depois, o `git status` do worktree estava vazio, o HEAD era o mesmo e as portas estavam livres. O worktree foi removido. Rastro bruto em `~/Dev/Skills/pstack-vic-runs/2026-09-25-parte-4a/`.

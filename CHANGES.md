...
- **Ninguém acompanha o PR depois de um arm com pendentes.** A Raiz encerra no recibo do arm, e o `start.ts` não lança owner para head certificado. O Varredor desarma, a cada `Tests` na `main`, o PR com hold e auto-merge pendente. Entre o arm e a varredura seguinte, um hold não para o auto-merge do GitHub, a não ser que o repositório torne o hold um check obrigatório. A frase do contrato sobre o owner que desarma num hold novo agora diz que vale para o fluxo do owner da nuvem.
- **Um Certificado é derivado de novo quando a ponta da `main` ou o texto do PR mudou.** O commit de contrato do Certificado é a ponta da `main` no `assemble`, e o Varredor roda depois de um push na `main`, então a ponta quase sempre mudou. A Automation de "PR opened" também pode comentar no PR entre a publicação e o arm. As lanes do Certificado nunca leem o texto do PR; a publicação decidiu sobre ele pela `analyze` e pela `decide`. Antes, o arm recusava todo `verdict` com commit de contrato diferente da ponta e todo PR com texto novo. Agora, para um dossiê `pre-pr`, o arm roda `certifiedAtTip` uma vez por arm, depois de conferir a ponta. Ela monta o snapshot do PR sob `pre-pr`, recusa se a ponta ou o texto mudou durante a leitura (`Trunk moved during re-derivation`, `PR text changed during re-derivation`), roda a mesma análise com o id do round e exige o mesmo head, o mesmo patch id estável e o mesmo digest de verificação (`Certificate patch or policy differs at trunk tip SHA`). Depois decide de novo sobre o corpo e os comentários atuais, com uma lane retida por papel exigido, com a cobertura e a adjudicação de risco do dossiê, como o `publish.ts --retain` faz, e recusa o que não sair VERIFIED (`Certificate is no longer VERIFIED at trunk tip SHA: VERDICT`). Um comentário comum passa; uma claim `check:` nova ou um comentário de injeção recusam. O resultado do arm ganha um passo que nomeia a nova derivação (`Re-derived the pre-pr verdict from contract OLD at trunk tip SHA`, `... over changed PR text`). Nada é publicado. Vale no modo estrito e no `--pending`. A nova derivação prende a ponta na hora do arm; sem proteção que exija a branch em dia, o GitHub pode mergear numa ponta posterior que ninguém derivou de novo, a mesma exposição que essa proteção já aceita para o código. Um dossiê `converge` continua recusado com commit de contrato antigo e com texto novo (`PR text changed after verification`), porque o verificador dele leu o texto. A forma da lane retida saiu para `retainedLanes` em `publish.ts`, que o `--retain` e o arm usam.
- **Um só teste de `verdict` confiável.** `statuses` foi para `github.ts`, e `verdictStatus` devolve o status `verdict` mais novo do head como confiável só quando é `success`, `VERIFIED by converge`, da conta autenticada e ligado a um comentário deste PR. Um VERIFIED postado por outra conta volta como estrangeiro, não como ausente. O arm, o Varredor e o `start.ts` usam o mesmo teste.
- **`converge-sweep --repo OWNER/REPO [--config PATH] [--dry-run]`.** Lista os PRs abertos com base na `main`, em ordem de número. Desarma o PR com hold e auto-merge pendente (`disarmed`); com `--dry-run`, só relata (`hold label, would disarm auto-merge`). Pula base diferente da `main`, hold sem auto-merge, rascunho, auto-merge já pendente e head sem `verdict` confiável VERIFIED. Recusa o PR cujo `verdict` VERIFIED mais novo foi postado por outra conta (`VERIFIED verdict status was posted by another account: LOGIN`), porque essa é a premissa de conta única da spec falhando. Roda `converge-arm --pending` em cada um dos outros. Imprime `{ swept: [{ pr, head, outcome, reason }] }`, com `outcome` `armed`, `disarmed`, `dry-run`, `skipped` ou `refused`. Uma recusa, ou uma falha ao ler o status de um PR ou ao desarmar, entra como `refused` e não para a varredura; a saída é 1 quando algum PR foi recusado. O Varredor não julga código e não comenta no PR. A Automation da parte 4 decide o que fazer com uma recusa.
...
- **`start.ts` sai sem lançar owner quando o head está certificado.** Depois de admitir o PR, se o `verdict` do head é confiável, lê o PR de novo, recusa se o head mudou (`PR head moved`) e devolve `{ schemaVersion: 1, kind: 'certified', repo, pr, head, verdictUrl }` sem chamar o Cursor e sem gravar intenção de lançamento. O caminho de lançamento devolve `{ kind: 'launched', ... }` com os campos do recibo, e o `launch.json` continua sem `kind`. Um `verdict` que não é VERIFIED, de outra conta ou ligado a outro PR ainda lança o owner. Recibo e intenção de lançamento já gravados continuam valendo antes dessa leitura.
...
- `npm test`: 507 testes, 0 falhas. A suíte do Converge tem 229, e 32 são desta parte: 20 em `arm.test.ts`, 7 em `sweep.test.ts` e 5 em `start.test.ts`.
...
- Os testes rodam os CLIs reais contra o `gh` falso. O `--pending` arma com checks na fila e o Tests do head ainda rodando, com a mutação exata `gh pr merge 1 --repo Example/app --squash --auto --match-head-commit HEAD`, onde o modo estrito recusa. Recusa `failure` e `neutral`. Sem checks ainda, recusa sem mutação hold, `main` vermelha, `verdict` fora da proteção, `verdict` não confiável, `verdict` de outra conta, corpo alterado num dossiê `converge` e base diferente da `main` (foco de revisão 4). Com `tests.job: 'test'`, o arm aceita um job `test` na `main`, que o contrato padrão recusa.
- Um Certificado `ci-only` e um `full` publicados antes de a `main` andar passam depois no arm estrito em dry-run e armam no `--pending`, com uma única leitura do snapshot por arm e o passo da nova derivação nos `steps`. Com a `main` andando, recusam sem mutação: um arquivo de política trocado, um arquivo sem patch legível (decisão não VERIFIED) e um dossiê `converge`. Com texto novo no PR, um Certificado arma sobre um comentário comum e recusa uma claim `check:` nova ou um comentário de injeção; um dossiê `converge` recusa o comentário novo.
- O Varredor arma o Certificado publicado antes de a `main` andar, e o `--dry-run` dele não faz mutação. Arma o PR certificado e pula o filho de stack, o com hold, o já armado e o rascunho; pula o PR sem `verdict` com saída 0; com a `main` vermelha devolve `refused` e saída 1; recusa com saída 1 um `verdict` VERIFIED de outra conta. Desarma o PR com hold e auto-merge pendente com a mutação exata `gh pr merge 1 --repo Example/app --disable-auto`, e o `--dry-run` só relata. O `start.ts` recusa, sem lançar, um head certificado que muda antes da saída.

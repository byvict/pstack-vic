...
Parte 2 do plano [`docs/superpowers/plans/2026-09-24-pre-pr.md`](docs/superpowers/plans/2026-09-24-pre-pr.md), sobre a spec [`docs/pre-pr.md`](docs/pre-pr.md). A Raiz arma o auto-merge logo depois de publicar o Certificado, sem esperar o CI do PR. O Varredor arma, depois de cada `Tests` na `main`, todo PR certificado que ainda não está armado, e desarma o PR armado que tem hold ou que não tem `verdict` confiável no head. O lançador do owner de "PR opened" encerra quando o head já tem um `verdict` confiável e o dossiê dele. A referência está em `skills/poteto-mode/references/converge-contract.md`.

## Desenho

- **`converge-arm --pending` arma com o CI do PR ainda rodando.** Não exige o run do Tests do head exato concluído nem cada check obrigatório verde. O arm lê o último check run de cada nome. Um check obrigatório cujo último run está na fila, em andamento ou esperando, ou que ainda não tem run, passa, e o GitHub segura o merge até ele passar. Um check obrigatório cujo último run, do app que a proteção exige, concluiu com conclusão diferente de `success` recusa (`Required protected check failed: NAME`), inclusive `neutral` e `skipped`, que o modo estrito também recusa. Dois casos ficam para a regra do GitHub na hora do merge: um contexto obrigatório postado só como commit status, e um run falho escondido por um run mais novo do mesmo nome vindo de outro app. O resto é igual ao modo estrito: Tests e job de teste na ponta da `main`, todo contexto do contrato na proteção efetiva, `verdict` confiável no head exato, PR aberto e pronto, com base `main`, no head dado e sem hold. Sem `--pending`, o arm não muda.
- **Ninguém acompanha o PR depois de um arm com pendentes.** A Raiz encerra no recibo do arm, e o `start.ts` não lança owner para head certificado. A cada `Tests` na `main`, o Varredor desarma o PR armado que tem hold ou que não tem `verdict` confiável no head. Entre o arm e a varredura seguinte, um hold não para o auto-merge do GitHub, a não ser que o repositório torne o hold um check obrigatório. Um push de quem tem escrita não desliga o auto-merge do GitHub, e o GitHub não documenta que o head informado ao ligar o auto-merge prende o merge. Por isso quem escreve desarma antes de empurrar para um PR armado. A frase do contrato sobre o owner que desarma num hold novo agora diz que vale para o fluxo do owner da nuvem.
- **A publicação recusa com auto-merge pendente.** A última leitura do PR antes de escrever recusa (`Auto-merge is pending on this PR; disarm it before publishing a verdict`). A Raiz publica antes de armar, e o owner da nuvem e o Reparo desarmam antes de uma rodada nova, então um auto-merge antigo nunca dispara sobre um status recém-publicado.
- **Um Certificado é derivado de novo em todo arm.** O commit de contrato do Certificado é a ponta da `main` no `assemble`, e o Varredor roda depois de um push na `main`, então a ponta quase sempre mudou. A Automation de "PR opened" também pode comentar no PR entre a publicação e o arm. As lanes do Certificado nunca leem o texto do PR; a publicação decidiu sobre ele pela `analyze` e pela `decide`. Antes, o arm recusava todo `verdict` com commit de contrato diferente da ponta e todo PR com texto novo. Agora, para um dossiê `pre-pr`, o arm roda `rederivePrePr` uma vez em todo arm, depois de conferir a ponta. Ela monta o snapshot do PR sob `pre-pr`, recusa se a ponta ou o texto mudou durante a leitura (`Trunk moved during re-derivation`, `PR text changed during re-derivation`), roda a mesma análise com o id do round e exige o mesmo head, o mesmo patch id estável e o mesmo digest de verificação (`Certificate patch or policy differs at trunk tip SHA`). Depois decide de novo sobre o corpo e os comentários atuais, com uma lane retida por papel exigido, com a cobertura e a adjudicação de risco do dossiê, como o `publish.ts --retain` faz, e recusa o que não sair VERIFIED (`Certificate is no longer VERIFIED at trunk tip SHA: VERDICT`). Sem nada mudado, o mesmo relatório com as lanes retidas sai VERIFIED de novo. Um comentário comum passa; uma claim `check:` nova ou um comentário de injeção recusam. O resultado do arm ganha um passo, também devolvido como `rederived`: `Re-derived the pre-pr verdict at trunk tip SHA`, com `from contract OLD` quando o commit de contrato difere e `over changed PR text` quando o texto difere. Nada é publicado. Vale no modo estrito e no `--pending`. A nova derivação prende a ponta na hora do arm; sem proteção que exija a branch em dia, o GitHub pode mergear numa ponta posterior que ninguém derivou de novo, a mesma exposição que essa proteção já aceita para o código. Um dossiê `converge` continua recusado com commit de contrato antigo e com texto novo (`PR text changed after verification`), porque o verificador dele leu o texto. A forma da lane retida saiu para `retainedLanes` em `publish.ts`, que o `--retain` e o arm usam.
- **Um só teste de `verdict` confiável, que lê o dossiê.** `statuses` foi para `github.ts`, e `verdictStatus` devolve o status `verdict` mais novo do head como confiável só quando é `success`, `VERIFIED by converge`, da conta autenticada e ligado a um comentário deste PR. Um VERIFIED postado por outra conta volta como estrangeiro, com o motivo. Um sem link, ou com link para outro PR, volta como ausente, e o arm recusa com `Verdict status does not link to this PR`. Um `target_url` ausente não derruba a leitura. `linkedDossier`, em `publish.ts`, lê o comentário do status e exige a conta autenticada e um dossiê VERIFIED `pre-pr` ou `converge` deste repositório, PR e head. O arm e o `start.ts` usam os dois, e o Varredor usa o primeiro.
- **`converge-sweep --repo OWNER/REPO [--config PATH] [--dry-run]`.** Lista os PRs abertos com base na `main`, em ordem de número. Pula base diferente da `main`. Desarma o PR com hold e auto-merge pendente; pula o com hold sem auto-merge e o rascunho. Com auto-merge pendente, pula o PR com `verdict` confiável no head e desarma o outro (`no trusted verdict on head` ou o motivo da conta estrangeira). Pula o PR sem `verdict` confiável e recusa o `verdict` VERIFIED de outra conta (`VERIFIED verdict status was posted by another account: LOGIN`), porque essa é a premissa de conta única da spec falhando. Roda `converge-arm --pending` em cada um dos outros. Um desarme é relatado pelo que se vê depois dele. O PR que não está mais aberto e o que continua armado são `refused`. O que já estava sem auto-merge, sem comando rodado, é `skipped`. Só um desarme observado é `disarmed`. Com `--dry-run`, só relata (`..., would disarm auto-merge`). Imprime `{ swept: [{ pr, head, outcome, reason }] }`, com `outcome` `armed`, `disarmed`, `dry-run`, `skipped` ou `refused`; num `armed` ou num arm em dry-run, o `reason` traz o passo da nova derivação de um `verdict` `pre-pr`. Uma recusa, ou uma falha ao ler o status de um PR ou ao desarmar, entra como `refused` e não para a varredura; a saída é 1 quando algum PR foi recusado. O Varredor não julga código e não comenta no PR. A Automation da parte 4 decide o que fazer com uma recusa.
- **O que o Varredor não resolve sozinho.** Depois de uma mudança de política confiável na `main`, toda varredura recusa o PR certificado até a Raiz certificá-lo de novo. Com a `main` vermelha, toda varredura recusa até um `Tests` da `main` passar. Um filho de stack não consegue publicar o Certificado enquanto a base é a branch do pai (`PR base differs from trunk`). Depois que o pai mergeia e o GitHub retargeta o filho para a `main`, a publicação daquele Certificado recusa na ponta nova (`Certificate patch or policy differs from the PR`). O Varredor pula o filho (`no trusted verdict on head`) até a Raiz certificá-lo e publicá-lo de novo.
- **`start.ts` sai sem lançar owner quando o head está certificado.** Depois de admitir o PR, se o `verdict` do head é confiável e `linkedDossier` confirma o dossiê dele, lê o PR de novo, recusa se o head mudou (`PR head moved`) e devolve `{ schemaVersion: 1, kind: 'certified', repo, pr, head, verdictUrl }` sem chamar o Cursor e sem gravar intenção de lançamento. Sem isso, segue o caminho de lançamento, que devolve `{ kind: 'launched', ... }` com os campos do recibo; o `launch.json` continua sem `kind`. Recibo e intenção de lançamento já gravados continuam valendo antes dessa leitura.
...
- `npm test`: 526 testes, 0 falhas. A suíte do Converge tem 248, e 51 são desta parte: 26 em `arm.test.ts`, 15 em `sweep.test.ts`, 8 em `start.test.ts` e 2 em `publish.test.ts`.
...
- Os testes rodam os CLIs reais contra o `gh` falso. O `--pending` arma com checks na fila e o Tests do head ainda rodando, com a mutação exata `gh pr merge 1 --repo Example/app --squash --auto --match-head-commit HEAD`, onde o modo estrito recusa. Recusa `failure`, `neutral` e `skipped`. Sem checks ainda, recusa sem mutação hold, `main` vermelha, `verdict` fora da proteção, `verdict` não confiável, de outra conta, ligado a outro PR ou sem link, corpo alterado num dossiê `converge` e base diferente da `main` (foco de revisão 4). Com `tests.job: 'test'`, o arm aceita um job `test` na `main`, que o contrato padrão recusa.
- Um Certificado `pre-pr` é derivado de novo em todo arm, uma vez (uma leitura de snapshot por arm), mesmo sem nada mudado, com o passo nos `steps`. Um `ci-only` e um `full` publicados antes de a `main` andar passam depois no arm estrito em dry-run e armam no `--pending`. Com a `main` andando, recusam sem mutação: um arquivo de política trocado, um arquivo sem patch legível (decisão não VERIFIED) e um dossiê `converge`. Com texto novo no PR, um Certificado arma sobre um comentário comum e recusa uma claim `check:` nova ou um comentário de injeção; um dossiê `converge` recusa o comentário novo. A `main` ou o texto mudando no meio da nova derivação recusam sem mutação.
- O Varredor arma o Certificado publicado antes de a `main` andar, com o passo da nova derivação no `reason`, e o `--dry-run` dele não faz mutação. Arma o PR certificado e pula o filho de stack, o com hold e o rascunho; pula o PR sem `verdict` ou com `verdict` sem link, com saída 0; com a `main` vermelha devolve `refused` e saída 1; recusa com saída 1 um `verdict` VERIFIED de outra conta. Desarma com a mutação exata `gh pr merge 1 --repo Example/app --disable-auto` o PR com hold e auto-merge pendente e o PR armado sem `verdict` confiável ou com `verdict` estrangeiro, e só pula o armado com `verdict` confiável; o `--dry-run` só relata. Um PR mergeado antes do desarme e um que continua armado depois dele saem `refused`; um que já estava sem auto-merge sai `skipped`.
- A publicação `converge` e a `pre-pr` recusam com auto-merge pendente, sem comentário nem status. O `start.ts` sai como certificado sobre um Certificado publicado de verdade. Lança o owner quando o status não tem comentário, quando o dossiê é de outro head, e quando o `verdict` não é VERIFIED, é de outra conta, está ligado a outro PR ou não tem link. Recusa, sem lançar, um head certificado que muda antes da saída.

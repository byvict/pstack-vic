...
Parte 2 do plano [`docs/superpowers/plans/2026-09-24-pre-pr.md`](docs/superpowers/plans/2026-09-24-pre-pr.md), sobre a spec [`docs/pre-pr.md`](docs/pre-pr.md). A Raiz arma o auto-merge logo depois de publicar o Certificado, sem esperar o CI do PR. O Varredor arma, depois de cada `Tests` na `main`, todo PR certificado que ainda não está armado, e desarma o PR armado que tem hold ou que o portão de veredito recusa. O lançador do owner de "PR opened" encerra quando o portão de veredito certifica o head. A referência está em `skills/poteto-mode/references/converge-contract.md`.

## Desenho

- **Um só portão de veredito decide pelo arm, pelo Varredor e pelo `start.ts`.** Três rodadas de revisão caíram na mesma premissa, a de que um status `verdict` basta para decidir algo sobre um PR. `verdictGate`, em `gate.ts`, substitui a premissa. Certifica só quando o status `verdict` mais novo do head é `success`, `VERIFIED by converge`, da conta autenticada e ligado a um comentário deste PR; esse comentário é da conta autenticada e é um dossiê VERIFIED `pre-pr` ou `converge` deste repositório, PR e head, conciliado contra o mesmo caminho de contrato (`Verdict was reconciled against another contract path`); nenhuma publicação mais nova o substitui; num `converge`, o commit de contrato é a ponta da `main` e o texto do PR não mudou; num `pre-pr`, a nova derivação passa. Senão recusa, com o motivo. Uma leitura do GitHub que falha não é veredito. `RequestError`, que `command` e `commandAsync` agora lançam, atravessa o portão. O arm passa pelo portão nas duas leituras do veredito e compara os resultados; o Varredor, em todo PR armado; o `start.ts`, antes de sair como certificado. `statuses` e `verdictStatus` ficam em `github.ts`, e o `start.ts` não importa mais `publish.ts`.
...
- **Ninguém acompanha o PR depois de um arm com pendentes.** A Raiz encerra no recibo do arm, e o `start.ts` não lança owner para head certificado. A cada `Tests` na `main`, o Varredor desarma o PR armado que tem hold ou que o portão recusa. Entre o arm e a varredura seguinte, um hold não para o auto-merge do GitHub, a não ser que o repositório torne o hold um check obrigatório. Um push de quem tem escrita não desliga o auto-merge do GitHub, e o GitHub não documenta que o head informado ao ligar o auto-merge prende o merge. Por isso quem escreve desarma antes de empurrar para um PR armado. A frase do contrato sobre o owner que desarma num hold novo agora diz que vale para o fluxo do owner da nuvem.
...
- `npm test`: 538 testes, 0 falhas. A suíte do Converge tem 260, e 63 são desta parte: 27 em `arm.test.ts`, 22 em `sweep.test.ts`, 10 em `start.test.ts` e 4 em `publish.test.ts`.
...
- O Varredor decide pela leitura viva do PR. Arma o Certificado publicado antes de a `main` andar, com o passo da nova derivação no `reason`, e o `--dry-run` dele não faz mutação. Arma o PR certificado e pula o filho de stack, o com hold e o rascunho; pula o PR sem `verdict` ou com `verdict` sem link, com saída 0; com a `main` vermelha devolve `refused` e saída 1; recusa com saída 1 um `verdict` VERIFIED de outra conta. Num PR armado, só pula o que o portão certifica, também com a `main` vermelha. Desarma com a mutação exata `gh pr merge 1 --repo Example/app --disable-auto` e saída 1 o PR armado sem `verdict`, com `verdict` estrangeiro, com `verdict` substituído por publicação mais nova, com comentário de injeção num Certificado, e o rascunho armado sem `verdict`; o `--dry-run` recusa sem mutação. Uma leitura de comentário que falha sai `refused` sem desarme. O desarme por hold sai `disarmed` com saída 0; um PR mergeado antes do desarme, também quando o comando de desarme falha, e um que continua armado saem `refused`; um que já estava sem auto-merge sai `skipped`.
- A publicação `converge` e a `pre-pr` recusam com auto-merge pendente, sem comentário nem status, e uma publicação nova depois de um arm também recusa; um retry byte-idêntico com auto-merge pendente passa sem escrever. O `start.ts` sai como certificado sobre um Certificado publicado de verdade. Lança o owner quando o status não tem comentário, quando o dossiê é de outro head, quando uma publicação mais nova substitui o `verdict`, e quando o `verdict` não é VERIFIED, é de outra conta, está ligado a outro PR ou não tem link. Falha, sem lançar e sem gravar intenção, quando a leitura do comentário falha, e recusa um head certificado que muda antes da saída.

...
| --- | --- | --- | --- |
| The program root, which runs the 30-minute audit tick, the operator chat, the swarm verdict aggregation and the countersigns. The root builds nothing, reviews nothing and runs no lane itself. | Claude Code | No self-scheduled wake-up in Grok Build 1.0.41. Its README names cron only as an external caller. | Appendix A |

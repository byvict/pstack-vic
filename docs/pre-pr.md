...
- Medido em 2026-09-25 (CLI-198, 0.2.4): pelo runner no modo `unsandboxed`, num worktree descartável do Clinext em `11b09bbae`, launch, doctor, `drive-login.mjs` e cleanup saem em 0, e o recibo fecha `complete` com `checkout` de HEAD igual e status vazio (sessão Grok `01a0d925-f116-7f63-8b1b-522736525367`). O overlay `inherit = "core"` corta o ambiente que o Grok repassa ao shell da lane, mas não o que o `~/.zshenv` exporta, que o zsh relê a cada comando: `CLAUDE_CODE_OAUTH_TOKEN`, `CURSOR_API_KEY` e `CLINEXT_TOKEN_COUNT_KEY` chegam à lane (nota N27). Rastro em `~/Dev/Skills/pstack-vic-runs/2026-09-25-parte-4a/`.

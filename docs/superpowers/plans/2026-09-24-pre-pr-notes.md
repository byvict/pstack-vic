...
- **N27 (CLI-198).** The `core` environment policy does not keep the credentials that `~/.zshenv` exports out of an `unsandboxed` lane's shell. In the Part 4a proof, the lane's shell listed `CLAUDE_CODE_OAUTH_TOKEN`, `CURSOR_API_KEY` and `CLINEXT_TOKEN_COUNT_KEY` (measured 2026-09-25, Grok CLI 1.0.41). Two control lanes in a scratch repository show that the overlay works for what the runner passes down. With the overlay, the shell saw 40 variable names instead of 91, and a marker variable set only in the runner's environment, `SSH_AUTH_SOCK` and `CLAUDE_CODE_MESSAGING_TOKEN` were gone. The three `~/.zshenv` exports reached the shell in both modes. The overlay variants `ignore_default_excludes = false` and `[toolset.bash] login_shell_capture = false` did not remove them either, because zsh reads `~/.zshenv` again for every command, after Grok builds the environment. The fix is on the machine, not in the runner. One option is to skip those exports in `~/.zshenv` when `GROK_AGENT` is set, which Grok sets in the lane's shell. Another is to move them to a file that only the Claude lanes read. Measure the chosen fix with the proof prompt's command 8 before Part 5 runs a certifier. Raw trail: `~/Dev/Skills/pstack-vic-runs/2026-09-25-parte-4a/env-probe/`.

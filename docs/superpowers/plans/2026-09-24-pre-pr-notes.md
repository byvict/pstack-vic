...
- **N24 (CLI-197).** Clinext #2966 (`11b09bbae`) made `launch` write nothing inside the checkout, and a Grok `read-only` lane now passes the write sites. It then stops at the next blocker, and no sandboxed Grok lane on macOS drives the app (measured 2026-09-25, Grok CLI 1.0.41, worktree `~/Dev/clinext-cli197-lane`). Grok's embedded Seatbelt profile allows `pseudo-tty` but grants `file-write*` only to its path list, so `openpty` fails with EPERM and tmux reports `create window failed: fork failed`; a custom profile with `read_write = ["/dev"]` fixes the pty. Chromium then dies with SIGSEGV inside IOKit (`IONotificationPortGetRunLoopSource` on a null port) and, once IOKit is allowed, on `bootstrap_check_in` (needs `mach-register`); custom profiles can express neither rule. With `--sandbox off` and the runner's argv otherwise unchanged, the same lane ran launch, doctor, `drive-login.mjs` and cleanup end to end: all exit 0, PNG and JSON evidence under `$TMPDIR`, HEAD unchanged, `git status` empty. The certifier therefore needs Grok without its sandbox: a new runner mode in a disposable worktree that the Raiz creates and checks afterwards (HEAD and `git status`), or T3 Code as the launcher, which runs the same unsandboxed Grok and adds no confinement. Victor decided on 2026-09-25 (CLI-197): mode `unsandboxed`, Grok only, `--cwd` a git worktree, HEAD and `git status` recorded in the receipt and checked at admission, environment policy by a `GROK_CONFIG_PATH` overlay. The plan's Part 4a carries it; N24 closes when its proof runs through the runner.
